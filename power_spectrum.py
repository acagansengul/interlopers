"""
`power_spectrum.py` adapted from Ana's Subhalo-correlations code
--------------------------------------------------------------------------------
Calculates the power spectrum from a 2d convergence.

Currently written for python2.
"""
...
ns = [0] # we just want the monopole contribution
power_spectra,K = multipoles(tot_ps,kx,ky,mask=mask,pix_num=pix_num,dr=pix_size_k,ns=ns)

K = K[1:]

dirk = dirk + 'k%s_%s_%s_%s.txt' % (name,numb,pix_num,rnge)
file0 = open(dirk,'w')
for i in K:
    file0.write('%s\n' % i)
file0.close()
