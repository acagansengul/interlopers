## from Ana's Subhalo-correlations code ##
...
import scipy
# import yt  ##CS EDIT
# from yt.analysis_modules.halo_analysis.api import *  ##CS EDIT
...
def simul_data(data,Rnge=600,mlow=1e6,mhigh=0):
    ...
    for i in data:
        file = yt.load(i)
        ad = file.all_data()
        h = file.hubble_constant
        z = file.current_redshift

        if len(file.field_list) != 0:
            masses.append(ad["halos","particle_mass"].in_units("Msun/h"))
            xs.append(ad["halos", "particle_position_x"].in_units("kpc/h"))
            ys.append(ad["halos", "particle_position_y"].in_units("kpc/h"))
            zs.append(ad["halos", "particle_position_z"].in_units("kpc/h"))
            rh.append(ad["halos", "rs"].in_units("kpc/h"))
            rvmax.append(ad["halos", "virial_radius"].in_units("kpc/h"))    ###### (SUBST.)LOOK AT THIS AGAIN!
    masses = [i for j in masses for i in j]
    xs = [i for j in xs for i in j]
    ys = [i for j in ys for i in j]
    zs = [i for j in zs for i in j]
    positions1 = [np.array([xs[i],ys[i],zs[i]]) for i in range(len(xs))]
    ...
    subh = list(zip(masses[1:],positions[1:],rh[1:],rvmax[1:]))
...
def simul_data_ANA(data,Rnge=600,mlow=1e5,mhigh=1e2):
    """
    takes in subhalo catalogs and returns subhalo masses, positions, halfmass and vmax radii
    inputs:
        data: halo catalog
...
def projections(positions,masses,rvmax,rh,rnge=100000000000000000,shift=0,num_proj=10):
    """
    inputs:
    ...
    while count < num_proj:
        ...
        proj_xy2 = [[[i[0],i[1]],j,k,l,np.linalg.norm(i)] for i,j,k,l in list(zip(rot_pos,masses,rvmax,rh)) if -rnge/2.+shift < i[0] < rnge/2.+shift and -rnge/2.+shift < i[1] < rnge/2.+shift]
        proj_xz2 = [[[i[0],i[2]],j,k,l,np.linalg.norm(i)] for i,j,k,l in list(zip(rot_pos,masses,rvmax,rh)) if -rnge/2.+shift < i[0] < rnge/2.+shift and -rnge/2.+shift < i[2] < rnge/2.+shift]
        proj_yz2 = [[[i[1],i[2]],j,k,l,np.linalg.norm(i)] for i,j,k,l in list(zip(rot_pos,masses,rvmax,rh)) if -rnge/2.+shift < i[1] < rnge/2.+shift and -rnge/2.+shift < i[2] < rnge/2.+shift]
